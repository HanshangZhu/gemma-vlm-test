...
def main(cfg):
    ...
    model.action_head = torch.compile(model.action_head)      # speed-up
    tokenizer = AutoTokenizer.from_pretrained(cfg.model_path)
    opt = torch.optim.AdamW(model.action_head.parameters(), lr=cfg.lr)
    ...
    for epoch in range(cfg.epochs):
        ...
        for img, prompt, act in dl:
            ...
            outputs = model(
                ...
                images=img,
                states=None  # We don't have state information in this dataset
            )
            pred = outputs['actions']            # (B, 20, 4) - full sequence
            act = act.unsqueeze(1).expand(-1, 20, -1)  # Expand target to match sequence length
            loss = F.mse_loss(pred, act)  # Compute loss over full sequence
